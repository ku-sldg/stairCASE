...
void msg_wait_callback(void)
{
    ...
    if (VQ_DRV_POLL(&key_send_virtqueue)) {
        handle_key_send_callback(&key_send_virtqueue);
    }
}
...
int run(void)
{
    ...
    err = camkes_virtqueue_device_init(&flagger_recv_virtqueue, 20);
    if (err) {
    ...
    err = camkes_virtqueue_driver_init(&flagger_send_virtqueue, 21);
    if (err) {
    ...
    // for key man!
    /* Initialise recv virtqueue */
    err = camkes_virtqueue_device_init(&key_recv_virtqueue, 22);
    if (err) {
    ...
    /* Initialise send virtqueue */
    err = camkes_virtqueue_driver_init(&key_send_virtqueue, 23);
    if (err) {
