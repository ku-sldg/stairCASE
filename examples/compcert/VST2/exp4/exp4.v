...
Definition _loop : ident := 56%positive.
Definition _main : ident := 57%positive.
Definition _myId : ident := 54%positive.
Definition _result : ident := 55%positive.
...
Definition f_main := {|
  ...
  fn_params := nil;
  fn_vars := nil;
  fn_temps := ((_result, tint) :: (_loop, tint) :: (_t'1, tint) :: nil);
  fn_body :=
(Ssequence
  (Ssequence
      ...
      (Ssequence
        (Swhile
          (Ebinop Ogt (Etempvar _loop tint) (Econst_int (Int.repr 0) tint)
            tint)
          (Sifthenelse (Ebinop Ogt (Etempvar _loop tint)
