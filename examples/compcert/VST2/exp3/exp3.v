...
Definition _condition : ident := 55%positive.
Definition _loop : ident := 57%positive.
Definition _main : ident := 58%positive.
Definition _myId : ident := 54%positive.
Definition _result : ident := 56%positive.
...
Definition f_main := {|
  ...
  fn_params := nil;
  fn_vars := nil;
  fn_temps := ((_condition, tint) :: (_result, tint) :: (_loop, tint) ::
               (_t'1, tint) :: nil);
  fn_body :=
(Ssequence
  (Ssequence
    (Sset _condition (Econst_int (Int.repr 1) tint))
    (Ssequence
      (Sset _result (Econst_int (Int.repr 0) tint))
      (Ssequence
        (Sset _loop (Econst_int (Int.repr 1) tint))
        (Ssequence
          (Swhile
            (Ebinop Ogt (Etempvar _loop tint) (Econst_int (Int.repr 0) tint)
              tint)
            (Sset _loop (Econst_int (Int.repr 0) tint)))
          (Ssequence
            (Scall (Some _t'1)
              (Evar _myId (Tfunction (Tcons tint Tnil) tint cc_default))
              ((Etempvar _result tint) :: nil))
...
((___builtin_ais_annot,
 ...
 (___builtin_debug,
   Gfun(External (EF_external "__builtin_debug"
     ...
     {|cc_vararg:=true; cc_unproto:=false; cc_structret:=false|})) ::
 (_myId, Gfun(Internal f_myId)) :: (_main, Gfun(Internal f_main)) :: nil).

Definition public_idents : list ident :=
(_main :: _myId :: ___builtin_debug :: ___builtin_nop ::
